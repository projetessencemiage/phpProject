				...
				if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
				{
					...
					foreach($PosArray as $Key => $Y2)
					{
						...
						{ 
							...
							else { $Color = $this->getRandomColor(); } 
						}
						...
						if ( $Y2 != VOID )
						{
							...
							if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
							{
								...
								$listeNote = array(0 => 'D', 33 => 'C', 67 => 'B', 100 => 'A');
								$note = $listeNote[number_format($Serie["Data"][$Key],0)];
								...
								if ( $DisplayPos == LABEL_POS_INSIDE && abs($TxtHeight) < abs($BarHeight) )
								{
									...
									if ( $DisplayOrientation == ORIENTATION_HORIZONTAL || $DisplayOrientation == ORIENTATION_AUTO )
									{
										...
										$this->drawText($CenterX,$CenterY,$this->scaleFormat($note,$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>TEXT_ALIGN_MIDDLEMIDDLE,"FontSize"=>$DisplaySize,"FontName"=>$DisplayFont));
										$Done = TRUE;
										 
									}
									else  $this->drawText($CenterX,$CenterY,$Caption,array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>TEXT_ALIGN_MIDDLEMIDDLE,"FontSize"=>$DisplaySize,"Angle"=>90));
								}
								else
								{
									if ( $Serie["Data"][$Key] >= 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
									$this->drawText($X+$XOffset+$XSize/2,$Y2-$Offset,$this->scaleFormat($note,$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align,"FontSize"=>$DisplaySize));
								}
